...
class Corpus(object):
    ...
    *dropout*: SPE dropout, default = 0
    ...
    def __iter__(self):
        # Reset the iterator state before returning
...
def learn_spe2vec(corpus, outfile=None,
    ...
    *method*: modeling method, choose from ['cbow', 'skip-gram']
    ...
    model = gensim.models.Word2Vec(corpus, vector_size=vector_size, window=window, min_count=min_count, workers=n_jobs, sg=sg,
                              **kwargs)
...
class SPE2Vec(object):
    ...
    def smiles2vec(self, smi, dropout=0, mode = None):
        '''
        ...
        if mode not in [None, 'average', 'sum', 'avg_pool', 'sum_pool']:
        ...
        if mode == None:
            return [self.model.wv[tok] for tok in tokens if tok in self.token_keys]
